...
const TUTORIAL_SCRIPT_VERSION = 6;
const TUTORIAL_RULES_BY_STEP_ID = {
  ...
  cast_spell_to_stack: { actor: 'player', turnOwner: 'player', activePlayer: 'player', phase: 'main1', requiredAction: 'Cast Lightning Bolt targeting Nicol Bolas', sourceCardOrEffect: 'Lightning Bolt', boardPrecondition: 'Lightning Bolt in hand; Mountain on battlefield as a red source; Nicol Bolas is a legal target', stackPrecondition: 'Stack empty before casting', expectedZoneChange: 'Lightning Bolt: hand -> stack', completionCondition: 'Lightning Bolt cast to stack', tutorialTargetAnchor: 'hand-area' },
  ...
  copy_stack_item: { actor: 'player', turnOwner: 'player', activePlayer: 'player', phase: 'main1', requiredAction: 'Use Reverberate-style copy on Lightning Bolt', sourceCardOrEffect: 'Reverberate targeting Lightning Bolt; tutorial explicitly grants the extra {R}{R} for this stack tool lesson', boardPrecondition: 'Lightning Bolt and Negate on stack; Mountain remains on battlefield; scripted lesson mana covers Reverberate', stackPrecondition: 'Negate above Lightning Bolt before copy; Bolt copy expected above both after action', expectedZoneChange: 'Lightning Bolt copy created on stack', completionCondition: 'Copy Stack Item action performed', tutorialTargetAnchor: 'stack-panel', teachesPriorityWithStack: true },
  ...
  cast_delver: { actor: 'player', turnOwner: 'player', activePlayer: 'player', phase: 'main1', requiredAction: 'Play Island if needed, then cast and resolve Delver of Secrets', sourceCardOrEffect: 'Delver of Secrets creature spell', boardPrecondition: 'Delver in hand; Island in hand or on battlefield as blue source', stackPrecondition: 'Stack empty before cast; Delver resolves before leaving main phase', expectedZoneChange: 'Island: hand -> battlefield if needed; Delver: hand -> stack -> battlefield', completionCondition: 'Cast/resolve Delver action', tutorialTargetAnchor: 'hand-area', teachesPriorityWithStack: true },
  ...
  clone_control: { actor: 'player', turnOwner: 'player', activePlayer: 'player', phase: 'main1', requiredAction: 'Use Clone or Give Control', sourceCardOrEffect: 'Clone copying a creature / Act of Treason stealing a creature with late-game scripted setup mana', boardPrecondition: 'Clone or Act of Treason effect is being represented after resolving with visible creatures', stackPrecondition: 'Effect resolved or resolving', completionCondition: 'Clone or control-change action', tutorialTargetAnchor: 'card-detail' },
...
const validateTutorialScriptRules = (steps = []) => {
  ...
  if (!(undoIndex >= 0 && replayIndex > undoIndex && boltIndex > replayIndex)) console.warn('[Tutorial rules] Lightning Bolt must come after Undo and a required replay_mountain step');
...
const TUTORIAL_SCRIPT_STEPS = withTutorialRules([
  ...
  { id: 'cast_spell_to_stack', chapter: 'Act 1 / Turn 1 — Main Phase', title: 'Lightning Bolt Enters the Stack', scene: 'A red spell ignites above your restored Mountain.', dialogue: 'Your red source exists again. Now even violence may proceed through the proper paperwork.', objective: 'Cast Lightning Bolt targeting Nicol Bolas.', hint: 'With Mountain on the battlefield, tap Lightning Bolt in your hand and choose Cast Spell or Cast + Target. Mana payment is manual; keep resources honest.', anchor: 'hand-area', completion: 'detect' },
  ...
  { id: 'copy_stack_item', chapter: 'Act 1 / Turn 1 — Main Phase', title: 'Copy the Bolt', scene: 'Your spell casts a second shadow as Bolas floods the lesson with borrowed red mana.', dialogue: 'Ah. Multiplication. Dangerous when discovered by the desperate.', objective: 'Represent Reverberate by copying Lightning Bolt on the stack.', hint: 'Reverberate normally costs {R}{R}; this scripted stack lesson explicitly grants that mana so you can learn Copy Stack Item without another land-drop detour.', anchor: 'stack-panel', completion: 'detect' },
  ...
  { id: 'cast_delver', chapter: 'Act 2 / Turn 2 — Main Phase', title: 'Cast Delver of Secrets', scene: 'An Island rises before a small wizard steps onto the board carrying a terrible secret.', dialogue: 'Small creatures are often containers for larger problems.', objective: 'Play Island if needed, then cast Delver of Secrets and resolve it onto the battlefield.', hint: 'Delver costs {U}. Play Island first if it is still in hand, then tap Delver, cast it, and resolve the stack item.', anchor: 'hand-area', completion: 'detect' },
  ...
  { id: 'target_system', chapter: 'Act 5 / Turn 5 — Main Phase', title: 'A Promise with Consequences', scene: 'A green line connects spell and creature.', dialogue: 'A target is a promise with consequences.', objective: 'Mark Giant Growth’s creature target.', hint: 'Forest is available for Giant Growth’s {G}; use Ability/Target tools because Giant Growth targets a creature while on the stack.', anchor: 'target-tools', completion: 'detect' },
  { id: 'attach_to_permanent', chapter: 'Act 5 / Turn 5 — Main Phase', title: 'Rancor Finds a Body', scene: 'Rancor coils around a creature.', dialogue: 'Attachments are relationships with rules text.', objective: 'Use Attach to permanent on Rancor or another attachment.', hint: 'Forest is available for Rancor’s {G}; use Attach to permanent after the Aura resolves onto a creature.', anchor: 'card-detail', completion: 'detect' },
  ...
  { id: 'clone_control', chapter: 'Act 5 / Turn 5 — Main Phase', title: 'Clone and Betrayal', scene: 'A creature looks at itself and makes a tactical error.', dialogue: 'Copies are lies that learned anatomy. Control changes are betrayal with timestamps.', objective: 'Open card detail and use Clone or Give Control.', hint: 'Late-game scripted setup has already paid for and resolved Clone or Act of Treason; use these tools when that effect copies a permanent or changes control.', anchor: 'card-detail', completion: 'detect' },
...
const GameBoard = ({ gameId, realUserId, displayName, onExit }) => {
  ...
  const ensureTutorialStepSetup = useCallback(async (stepId) => {
    ...
    if (['replay_mountain', 'cast_spell_to_stack', 'inspect_stack', 'bolas_negate', 'copy_stack_item', 'resolve_stack_item', 'counter_stack_item', 'pass_priority'].includes(stepId)) {
