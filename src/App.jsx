...
const TUTORIAL_SCRIPT_VERSION = 5;
const TUTORIAL_RULES_BY_STEP_ID = {
  ...
  undo_play_land: { actor: 'player', turnOwner: 'player', activePlayer: 'player', phase: 'main1', requiredAction: 'Undo land play then replay it', sourceCardOrEffect: 'Tutorial demonstrates undo immediately after a reversible land play', boardPrecondition: 'Mountain on battlefield from this turn', stackPrecondition: 'Stack empty', expectedZoneChange: 'Mountain: battlefield -> hand -> battlefield', completionCondition: 'Undo action completed', tutorialTargetAnchor: 'undo-button' },
  cast_spell_to_stack: { actor: 'player', turnOwner: 'player', activePlayer: 'player', phase: 'main1', requiredAction: 'Cast Lightning Bolt targeting Nicol Bolas', sourceCardOrEffect: 'Lightning Bolt', boardPrecondition: 'Lightning Bolt in hand; Nicol Bolas is a legal target', stackPrecondition: 'Stack empty before casting', expectedZoneChange: 'Lightning Bolt: hand -> stack', completionCondition: 'Lightning Bolt cast to stack', tutorialTargetAnchor: 'hand-area' },
  ...
  copy_stack_item: { actor: 'player', turnOwner: 'player', activePlayer: 'player', phase: 'main1', requiredAction: 'Use Reverberate-style copy on Lightning Bolt', sourceCardOrEffect: 'Reverberate targeting Lightning Bolt', boardPrecondition: 'Lightning Bolt and Negate on stack', stackPrecondition: 'Negate above Lightning Bolt before copy; Bolt copy expected above both after action', expectedZoneChange: 'Lightning Bolt copy created on stack', completionCondition: 'Copy Stack Item action performed', tutorialTargetAnchor: 'stack-panel', teachesPriorityWithStack: true },
  ...
  counter_stack_item: { actor: 'player', turnOwner: 'player', activePlayer: 'player', phase: 'main1', requiredAction: 'Resolve Negate / counter original Bolt', sourceCardOrEffect: 'Negate targeting original Lightning Bolt', boardPrecondition: 'Negate and original Lightning Bolt remain on stack', stackPrecondition: 'Negate can counter original Lightning Bolt; stack should be cleared before phase changes', expectedZoneChange: 'Negate and original Bolt leave stack', completionCondition: 'Counter/Fizzle or resolve relevant stack item', tutorialTargetAnchor: 'stack-panel', teachesPriorityWithStack: true },
  ...
  game_log: { actor: 'player', turnOwner: 'player', activePlayer: 'player', phase: 'end', requiredAction: 'Open log/chat', sourceCardOrEffect: 'Async communication', boardPrecondition: 'Log available', stackPrecondition: 'Stack empty', completionCondition: 'Log/chat opened', tutorialTargetAnchor: 'chat-button' },
  ...
  cast_delver: { actor: 'player', turnOwner: 'player', activePlayer: 'player', phase: 'main1', requiredAction: 'Cast and resolve Delver of Secrets', sourceCardOrEffect: 'Delver of Secrets creature spell', boardPrecondition: 'Delver in hand', stackPrecondition: 'Stack empty before cast; Delver resolves before leaving main phase', expectedZoneChange: 'Delver: hand -> stack -> battlefield', completionCondition: 'Cast/resolve Delver action', tutorialTargetAnchor: 'hand-area', teachesPriorityWithStack: true },
  ...
  create_token: { actor: 'player', turnOwner: 'player', activePlayer: 'player', phase: 'main1', requiredAction: 'Create Goblin tokens', sourceCardOrEffect: 'Dragon Fodder', boardPrecondition: 'Dragon Fodder resolved', stackPrecondition: 'Token-making spell/effect resolved', expectedZoneChange: 'Goblin tokens created on battlefield', completionCondition: 'Create token action', tutorialTargetAnchor: 'token-tools' },
  ...
  target_system: { actor: 'player', turnOwner: 'player', activePlayer: 'player', phase: 'main1', requiredAction: 'Mark a target', sourceCardOrEffect: 'Giant Growth targeting a creature', boardPrecondition: 'Giant Growth/effect on stack and creature target exists', stackPrecondition: 'Targeting spell/effect on stack', completionCondition: 'Target marker action', tutorialTargetAnchor: 'target-tools', teachesPriorityWithStack: true },
  attach_to_permanent: { actor: 'player', turnOwner: 'player', activePlayer: 'player', phase: 'main1', requiredAction: 'Attach Rancor to creature', sourceCardOrEffect: 'Rancor Aura enchanting a creature', boardPrecondition: 'Rancor and creature on battlefield', stackPrecondition: 'Aura spell resolved', completionCondition: 'Attach card action', tutorialTargetAnchor: 'card-detail' },
  attach_to_player_note: { actor: 'player', turnOwner: 'player', activePlayer: 'player', phase: 'main1', requiredAction: 'Attach Curse to Bolas', sourceCardOrEffect: 'Curse of the Pierced Heart enchanting a player', boardPrecondition: 'Curse on battlefield and Bolas is enchanted player', stackPrecondition: 'Curse spell resolved', completionCondition: 'Attach to player action', tutorialTargetAnchor: 'card-detail' },
  clone_control: { actor: 'player', turnOwner: 'player', activePlayer: 'player', phase: 'main1', requiredAction: 'Use Clone or Give Control', sourceCardOrEffect: 'Clone copying a creature / Act of Treason stealing a creature', boardPrecondition: 'Clone or Act of Treason effect is being represented', stackPrecondition: 'Effect resolved or resolving', completionCondition: 'Clone or control-change action', tutorialTargetAnchor: 'card-detail' },
  ...
  final_spell: { actor: 'player', turnOwner: 'player', activePlayer: 'player', phase: 'main1', requiredAction: 'Cast lethal Lightning Bolt', sourceCardOrEffect: 'Lightning Bolt', boardPrecondition: 'Lightning Bolt in hand; Bolas at lethal range', stackPrecondition: 'Stack empty before casting', expectedZoneChange: 'Lightning Bolt: hand -> stack', completionCondition: 'Final spell cast', tutorialTargetAnchor: 'hand-area' },
  ...
  async_oath: { actor: 'player', turnOwner: 'player', activePlayer: 'player', phase: 'end', requiredAction: 'Open log/chat and pass priority', sourceCardOrEffect: 'Async table etiquette after duel', boardPrecondition: 'Duel ending', stackPrecondition: 'Stack empty', completionCondition: 'Log/pass action', tutorialTargetAnchor: 'chat-button' },
  ...
  manual_toolbox_note: { actor: 'player', turnOwner: 'player', activePlayer: 'player', phase: 'end', requiredAction: 'Open log/chat notes', sourceCardOrEffect: 'Manual tabletop responsibility', boardPrecondition: 'Game can continue as manual board', stackPrecondition: 'Stack empty', completionCondition: 'Log/chat focused', tutorialTargetAnchor: 'chat-button' },
...
const TUTORIAL_SCRIPT_STEPS = withTutorialRules([
  ...
  { id: 'copy_stack_item', chapter: 'Act 1 / Turn 1 — Main Phase', title: 'Copy the Bolt', scene: 'Your spell casts a second shadow.', dialogue: 'Ah. Multiplication. Dangerous when discovered by the desperate.', objective: 'Represent Reverberate by copying Lightning Bolt on the stack.', hint: 'Bolas has Negate above your Bolt. Open Lightning Bolt’s stack item and use Copy Stack Item to represent Reverberate creating a Bolt copy.', anchor: 'stack-panel', completion: 'detect' },
  ...
  { id: 'counter_stack_item', chapter: 'Act 1 / Turn 1 — Main Phase', title: 'Negate Closes', scene: 'Negate counters the original spell after the copy has resolved.', dialogue: 'The first future dies. Remember: the top resolves first.', objective: 'Resolve Negate or counter/fizzle the original Lightning Bolt so the stack is empty.', hint: 'The copied Bolt dealt damage. Now clear Negate and the original Bolt before changing phases.', anchor: 'stack-panel', completion: 'detect' },
  ...
  { id: 'transform_card', chapter: 'Act 3 / Turn 3 — Upkeep Trigger', title: 'Transform Delver', scene: 'The wizard’s shadow unfolds into wings.', dialogue: 'Identity is just a face with ambition.', objective: 'Transform Delver into Insectile Aberration.', hint: 'Open Delver’s card detail and use Transform / Switch face.', anchor: 'card-detail', completion: 'detect' },
  ...
  { id: 'phase_card', chapter: 'Act 2 / Bolas Turn — In Response', title: 'Slip Out the Back', scene: 'Delver folds sideways out of reality while Doom Blade waits on the stack.', dialogue: 'Annoying. You have found the door marked not here.', objective: 'Resolve Slip Out the Back by phasing out Delver.', hint: 'Doom Blade targets Delver. Open Delver details and use Phase Out because Slip Out the Back resolves first.', anchor: 'card-detail', completion: 'detect' },
  ...
  { id: 'tap_card', chapter: 'Act 3 / Turn 3 — Untap Step', title: 'Tap and Untap', scene: 'The phased-out creature returns as your untap step begins.', dialogue: 'Turn it sideways. The oldest gesture of usefulness.', objective: 'Tap and untap a permanent.', hint: 'Open card details and use Tap/Untap.', anchor: 'card-detail', completion: 'detect' },
  { id: 'face_down_reveal', chapter: 'Act 4 / Turn 4 — Hidden Information', title: 'Face Down and Reveal', scene: 'Bolas places a blank mask over a card.', dialogue: 'Sometimes a card is a secret with corners.', objective: 'Use Turn Face Down / Reveal to represent a Cloak or Morph-style effect.', hint: 'This is legal only when an effect creates hidden information; the tutorial frames it as a resolved Cloak-style effect.', anchor: 'card-detail', completion: 'detect' },
  ...
  { id: 'reveal_hand_note', chapter: 'Act 4 / Turn 4 — Public Knowledge', title: 'Reveal and Hide Hand', scene: 'Bolas opens one hand to the arena, then closes it.', dialogue: 'Public reveal is not the same as private knowledge. Do not confuse theatre with espionage.', objective: 'Use Reveal tools for a spell that makes a hand public to the table.', hint: 'Public reveal is justified only by an effect that tells everyone to see the hand.', anchor: 'reveal-tools', completion: 'detect' },
  ...
  { id: 'target_system', chapter: 'Act 5 / Turn 5 — Main Phase', title: 'A Promise with Consequences', scene: 'A red line connects spell and victim.', dialogue: 'A target is a promise with consequences.', objective: 'Mark Giant Growth’s creature target.', hint: 'Use Ability/Target tools because Giant Growth targets a creature while on the stack.', anchor: 'target-tools', completion: 'detect' },
  ...
  { id: 'player_counters', chapter: 'Act 6 / Turn 6 — Player Counters', title: 'Invisible Ledgers', scene: 'Poison, energy, and experience ledgers open beside your name.', dialogue: 'Some numbers sit on players because cards are too fragile to hold them.', objective: 'Add and remove a player counter caused by poison, energy, or experience effects.', hint: 'Use Poison after a toxic hit, Energy after Attune with Aether, or Experience after a Meren/Ezuri-style effect.', anchor: 'player-counters-panel', completion: 'detect' },
  { id: 'statuses', chapter: 'Act 6 / Turn 6 — Statuses', title: 'Crowns and Nightfall', scene: 'A crown, a dungeon door, a city seal, and a changing sky appear.', dialogue: 'Titles are counters with better costumes.', objective: 'Toggle a status caused by a monarch, initiative, day/night, City’s Blessing, or Ring-tempts effect.', hint: 'These are player/game statuses created by cards, not random errands.', anchor: 'status-panel', completion: 'detect' },
  { id: 'emblems', chapter: 'Act 6 / Turn 6 — Emblems', title: 'A Scar the Rules Remember', scene: 'A planeswalker’s threat remains after the planeswalker is gone.', dialogue: 'An emblem is a scar the rules refuse to forget.', objective: 'Add an emblem after a planeswalker ultimate resolves.', hint: 'Use preset, custom, or deck-derived emblems only when an effect creates one.', anchor: 'player-counters-panel', completion: 'detect' },
  { id: 'dungeons_note', chapter: 'Act 6 / Turn 6 — Dungeons', title: 'Checklist with Walls', scene: 'A map appears beneath the turn order.', dialogue: 'Dungeons are not places. They are checklists with walls.', objective: 'Open the dungeon reference for a venture or initiative effect.', hint: 'Use dungeon references when a card tells you to venture or take the initiative. Progress is manual.', anchor: 'player-counters-panel', completion: 'detect' },
...
const GameBoard = ({ gameId, realUserId, displayName, onExit }) => {
  ...
  const ensureTutorialStepSetup = useCallback(async (stepId) => {
    ...
    if (['inspect_stack', 'bolas_negate', 'copy_stack_item', 'resolve_stack_item', 'counter_stack_item', 'pass_priority'].includes(stepId)) {
    ...
    if (forcedTurnPlayerId && game.turnPlayerId !== forcedTurnPlayerId) updates.turnPlayerId = forcedTurnPlayerId;
